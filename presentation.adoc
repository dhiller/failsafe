...
```java
javaBackendReturnCodeTest.renameTo("BackendReturnCodeIT");
```
...
Abschliessend wird der Spring-Kontext der ``*Test`` Klasse soweit wie moeglich ausgeduennt oder der Test-Runner auf ``JUnit4.class`` gesetzt und die echten Abhaengigkeiten der ``*Test`` Klasse werden durch Mocks ersetzt.
